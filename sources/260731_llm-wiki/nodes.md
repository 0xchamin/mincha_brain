...
| n10 | **EFFICACY, UNMEASURED - an index file replaces embedding RAG at moderate scale.** "This works surprisingly well at moderate scale (~100 sources, ~hundreds of pages) and **avoids the need for embedding-based RAG infrastructure.**" | S8 (gist @ac46de1, §Indexing and logging) | (none) | single-leg | **needs-check - do not cite as a result**, and **researched 2026-08-15, verdict `refines`** - see [`context/01_where-the-index-file-ceiling-actually-sits.md`](context/01_where-the-index-file-ceiling-actually-sits.md). Still the document's one falsifiable claim and still arriving with nothing attached: no eval set, no comparison, no definition of "works well", no derivation of the ~100. "Surprisingly" is doing the work a measurement should. **What research changed:** the ceiling is **real and independently measured twice** (LOFT, T3 Google DeepMind, degrading positionally between 128k and 1M tokens; LlamaIndex, T2, crossing over at ~100 papers), and those two **agree with each other at roughly 1.5M tokens** - so **`n10`'s unit is wrong and its magnitude is a coincidence of its author's corpus.** What binds is tokens the navigation ranges over, not source count. **And neither study tested `n10`'s actual design**, which reads a summary index rather than the corpus and so attends to ~2k tokens instead of ~1.5M - `no-evidence` on the mechanism itself. **Confidence unchanged; the reason for it is better** |

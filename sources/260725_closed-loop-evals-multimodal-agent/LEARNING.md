...
**How to read it:** top to bottom is the order of the argument, grouped into four movements. The
**blue block is the core technique** - the single idea the talk exists to deliver. The **amber block
is what nobody tells you until you have shipped**: the parts that only matter once the system is live
and the world starts moving under it.

**The crux: sections 5, 6 and 7 are three different answers to the same question, and which one you
need depends on what the stage does - not on what the pipeline does.**

**Why it is grouped this way:** A and B are prerequisites you can skim if you already build agent
pipelines; C is the transferable payload; D is where most teams' understanding stops short, because
you cannot discover it from a design document, only from operating something. The arrows between
groups are strict - C makes no sense without B's decision points, and D is entirely about defending
what C measures.
...
A test suite asserts a known-correct answer, runs on every commit, and is green or red. Almost
nothing in this talk works like that. The system under test is **non-deterministic**; its output is
an **image**, for which no correct answer exists; the quality bar is partly **subjective** (does this
look appetising? does it still look like *that* restaurant's food?); and the thing you are defending
...
sounds like a straightforward image-enhancement job until you add the constraint that shapes
everything: **eaters distrust anything that looks AI-generated.** So an edit must stay **faithful**
to the actual dish, preserve each restaurant's **brand**, and avoid **sameness** - one prompt applied
to every photo would make the whole marketplace look identical, which is a business problem rather
than an aesthetic one `&t=169s`.
...
Those three goals close off both ends of the design space. A deterministic rules engine is
controllable and predictable, which serves *ship safely* - but it is brittle and will not survive
every dish, cuisine and lighting condition, so it fails *scale*. A fully agentic system is creative
and high-agency, which serves *scale* - but pointed unconstrained at a live marketplace it fails
*ship safely*, and it is exactly the thing that produces the AI-looking output that fails
*authenticity*. What is left is the **guardrailed middle** (`n14`, `&t=251s`).
...
cannot be made safe by evals, because measuring after the fact does not constrain action. Only the
guardrailed middle both *needs* measurement and *can be improved by* it. **The architecture and the
eval strategy are therefore one design decision, not two** - which is why the next thing to look at
is the architecture.
...
is a routing decision (enhance this, or leave it alone), a QA decision (publish, or retry), and a
final publish decision. Three places where the system commits to something it could have done
differently - and **each one fails in a different way**, which is precisely why no single number can
describe the whole.
...
> decomposed one is **attributable** - when quality drops you can localise the stage. Here
...
from what the system did, which means something must have recorded it - and the talk is emphatic
...
answer each part separately and leave you stitching timestamps; one flat record makes the whole
journey legible in a single read. Notice this is also what makes section 3's attribution argument
real rather than theoretical: a decomposed architecture only buys you localisation if the evidence
arrives already correlated.

> **Background, supplied.** In ordinary distributed systems this is the distinction between **logs**
> and **traces** - logs are per-service events, a trace is one request's whole path under a shared
> correlation ID. The idea is old and the tooling is mature. What is new here is the *consumer*: this
> trace is not only for a human debugging an incident, it is the **training input for the auto-tuning
> loop** in section 9. That is why "log first" is an ordering claim and not an instrumentation
> preference - **the loop cannot exist before its data does.**

With the trace in place, work through the decision points in order. The first one is routing, and it
is the friendliest, because it is the only stage where the right answer is a label a human could have
written down.
...
classification has been measured the same way for decades: a **confusion matrix**, with **precision
and recall** (`n3`, `&t=459s`). A router with more than two branches is no different in kind - it
becomes an **n x n** matrix, one cell per branch.
...
> **precision** asks: of the things you flagged, what share should have been flagged - how much of
> your action was wasted. **Recall** asks: of the things that should have been flagged, what share
> you caught - how much you missed. The two trade off, and trivially so: you can catch everything by
> flagging everything, at zero precision. Classical statistics names the two errors **Type I** (false
> positive) and **Type II** (false negative), and the entire discipline exists because **the two
> errors almost never cost the same.** So the engineering question is never "which is more accurate".
> It is **"which error can I afford"**.
...
goal from section 2 failing in the most visible way available. **So recall is the guardrail** -
optimise so that no bad image slips through (`n4`, `&t=578s`).
...
All of which assumes you can say what "bad" means. That comes from a **golden dataset**: human labels
as the source of truth, over a set representative across geography, dish type and quality, with
**objective guidelines** written to strip subjective variation between labellers (`n6`, `&t=528s`).
⚠️ `single-leg` - narration only, no slide captured, so read the practice as reported rather than
demonstrated. Hold onto the golden set; it becomes load-bearing again in section 9 for a reason
nobody expects.
...
> item and a loose rubric will disagree, which puts a hard ceiling on any metric computed from their
> labels. The talk names the remedy and never mentions measuring the agreement, which is the usual
> way you discover whether the remedy worked.
...
transfer. What the talk does instead is make the QA gate **explain itself**, and turn the explanation
...
> generation** benchmarking, where it measures *independent* samples: draw k completions, pass if any
> one compiles and passes the tests. There, k is measuring the model's **diversity** - you are buying
> attempts and hoping one lands. Uber's usage differs in a way the talk never flags: **each attempt
> is conditioned on why the last one failed**, so the curve measures the **feedback loop's**
> effectiveness rather than the model's spread. Two consequences follow. A rising curve here is
> evidence the QA reasoning is *useful*, which the independent-sampling version cannot tell you. And
> the two numbers are **not comparable** - never benchmark this pass@k against a published one.

That distinction is not pedantry, because it dictates the wiring. The retry has to re-enter at
**prompt generation**, not at generation, and the QA gate has to emit a *reason* rather than a
...
So generation gets a metric. But look again at what this system actually does to an image: it does
not create one, it **edits** one - and an edit has a reference that a creation never has.
...
Two details carry the whole idea. **"Did anything regress?" is a question absolute scoring cannot
ask** - a 7-out-of-10 tells you nothing about whether the plating improved while the colour got
worse, and regressions are exactly what an editing pipeline must not ship. And the **"unsure" option**
matters more than it looks: forcing a binary out of a judge that genuinely cannot tell manufactures
confidence you do not have, and those undecided cases are precisely the ones worth a human's
attention.
...
> editing task you do not need a second candidate to compare against - **the input is already
> there, and it is free.**

Sections 5 to 7 complete the core technique: three decision points, three failure modes, three
metrics chosen to match. That is the transferable payload, and if the world held still it would be
enough. The rest of this note is about the fact that it does not.
...
That duplication is deliberate. It is the **Swiss-cheese model** - every gate has holes, but if the
holes sit in different places, very little passes through all of them.
...
> is a warning as much as a technique: the layers must fail **independently**. Barriers that share a
> cause - the same bad assumption, the same training data, **the same judge model with a different
> prompt** - have their holes already aligned, and the stack is no stronger than one layer while
> looking considerably stronger. **The talk does not raise this**, and it is the first question to
> ask of any two-gate design.
...
only blocks bad output is a filter; one that attributes the miss is a diagnostic - and attribution is
the raw material for everything in the next section.
...
> input distribution moving: new cuisines, new phone cameras, seasonal dishes. **Concept drift** is
> the *relationship* moving: what counts as an acceptable photo changes because eater expectations
> changed, while the inputs look much the same. The first is detectable by watching inputs alone;
> **the second is visible only through fresh labels.** That the loop re-labels sampled production
> traffic rather than merely monitoring distributions tells you which threat it is built for, though
> the talk never separates the two.
...
and isolates the *systemic* issue while discarding noise, and a **synthesize** agent rewrites the
config from that finding. The new version lands in an **agent store**, gets picked up on the next
run, and carries observability and quick rollback. The split matters - finding the pattern and
writing the fix are different jobs, and merging them is how a config gets rewritten to chase a single
bad example.

Above that sits a **diagnoser** (`n13`, `&t=1144s`), which generalises the whole arrangement: rather
than one tuner bolted to each agent, a higher-level abstraction ingests *any* feedback signal, works
out **which** agent is responsible, and routes the fix there. This is what lets the loop scale past a
handful of stages, since otherwise every new agent needs its own bespoke tuning path.

> **Background, supplied.** Rewriting a prompt automatically from labelled failures is an active
> research area with a name and toolchains - **automatic prompt optimisation**, of which DSPy is the
> best-known framework. Two things are worth carrying: the idea is not exotic, and published methods
> are typically evaluated against a *fixed* benchmark whereas this loop runs against a **moving**
> distribution, which is strictly harder.

Which is exactly why the golden set from section 5 comes back. The loop rewrites configs on its own,
so something fixed has to stop it drifting somewhere worse; benchmarking every new version against a
stable human-labelled set is that anchor. **An auto-tuning loop with no fixed reference optimises
itself away from the truth, confidently** - it is a control system with no setpoint.
...
Each catches what the others structurally cannot. The **model loop** is fast and automated but can
only measure what someone already thought to measure. The **dogfooding loop** catches the qualitative
"this looks wrong somehow" that no metric encodes, at the cost of being slow and low-volume. The
**marketplace loop** is the only one that answers whether any of this made a **business** difference,
and it is the slowest and noisiest of the three.
...
> wants and least gameable, and also the one you can least often run. **The layering is not
...
QA gate*, and "make it blander" is a dependable way to stop failing a faithfulness check. **A
feedback loop rewards what it measures, and safety is usually cheapest to buy by doing less.** The
system did what it was asked; the asking was wrong.

> **Background, supplied.** This is **specification gaming**, also called reward hacking - among the
> most robustly observed behaviours in optimisation, where a system satisfies the stated objective
> while violating the intent, and does so *more* reliably as it gets better at optimising. The
> practical implication is unwelcome and worth internalising early: **it is not a bug you fix, it is
> a pressure you contain.** You contain it with an objective the optimiser cannot reach - which is
> what the marketplace loop is for - and by watching specifically for outputs that changed a great
> deal while improving nothing.

Which closes the argument where it started. Section 1 said you cannot evaluate this with one number;
section 11 shows what happens when you try, because a single QA gate *is* one number, and the agent
found its edge. The whole architecture below is that answer drawn out.

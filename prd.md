...
2. **Capture visuals (pre-filter in the shell, not by the agent):** `ffmpeg` samples on
   **scene-change** (`select='gt(scene,0.4)'`) with a 2s floor, then de-dup with `imagehash`
   ...
   - ⚠️ **`STATIC` is advisory, not dispositive** ([ADR-0006](brain/decisions/0006-static-probe-is-advisory.md)).
     Scene detection measures **whole-frame** delta, so a **templated conference deck** (fixed
     background, logo, speaker inset, footer, slide body a minority of the pixels) reads as static
     while every slide changes - observed twice. On `STATIC` the probe writes a **9-frame
     confirmation sheet**; **`view` it before honouring the verdict**, and override if the slides
     differ. The asymmetry justifies the call: a false `RICH` wastes one `view`, a false `STATIC`
     destroys the source's second leg irrecoverably.
...
| **Probe authority** ([ADR-0006](brain/decisions/0006-static-probe-is-advisory.md)) | `STATIC` is **advisory**, confirmed by a **9-frame sheet** costing one `view`; the constants stay frozen | A templated conference deck defeats whole-frame scene detection - it read as static on ~20 changing slides. **The two errors are asymmetric:** a false `RICH` wastes one `view`, a false `STATIC` destroys the source's second leg and cannot be undone (nodes may never be retro-marked `corroborated`). Confirming a verdict is not recomputing it, so ADR-0005's comparability survives. | v0.17 |
...
| R10 | **Two of the four source types are still unexercised** - 5 ingests to date (4 video, 1 blog); **paper** and especially **code** (`MAP.md`, `path:line @sha`, docs↔code divergence, generated diagrams) are prose that has never run | Known and accepted. **First contact reliably finds bugs, three times now:** a swallowed `ffmpeg` exit code in `tools/ingest.py`; the blog flow's first run (`260725_harness-design-long-running-apps`) finding that `web_fetch` declines verbatim reproduction and that outcome screenshots are the blog analogue of a static video; and a **false `STATIC`** on a templated deck ([ADR-0006](brain/decisions/0006-static-probe-is-advisory.md)). Treat the code flow's first ingest as a shakedown run, not a routine one. |
| R11 | **The pre-filters are tuned for whole-frame change, and conference recordings are composited** - fixed background, logo, speaker inset and footer around a minority-of-pixels slide body. Both the scene-detect and the pHash stages have now under-detected on this input class | [ADR-0006](brain/decisions/0006-static-probe-is-advisory.md): `STATIC` is **advisory**, confirmed by a 9-frame sheet costing one `view`, on the branch that would otherwise discard the whole visual leg. Additive - the constants are untouched, so past verdicts stay comparable. A third occurrence at a stage this does not cover should force a rethink of the metric itself. |
...
- **OQ3: the seed topic set is now moot as a *question*** - v0.6 made the topic set **open**, with
  ...
  corroborating source. The live question is narrower: **at what point does `agents.md` want
  splitting?** It is now **~250 lines across 4 sources** (was 170 across 3 when this was written),
  and the pressure is visible: it carries the four owned parts, micro agents, state and pause/resume,
  *and* the whole scaffolding-as-expiring-bets thread. That last one is the natural fault line.
...
| 2026-07-26 | v0.17 | **Made the static probe's `STATIC` verdict advisory** ([ADR-0006](brain/decisions/0006-static-probe-is-advisory.md)), and reconciled the docs with five ingests. **The defect:** scene detection measures *whole-frame* delta, so a templated conference deck - fixed background, logo, speaker inset, footer, slide body a minority of the pixels - reads as static while every slide changes. `260726_dont-ship-skills-without-evals` probed `candidates=3` on ~20 dense slides; `260725_12-factor-agents` hit the same root cause at the pHash stage. **The fix is additive, not a retune:** on `STATIC` only, `probe` now writes a 9-frame chronological confirmation sheet to `view` before honouring the verdict. The three constants are untouched, so every past verdict stays comparable - **confirming a verdict is not recomputing it**, which is what lets this coexist with ADR-0005. Justified by asymmetry: a false `RICH` costs one `view`, a false `STATIC` destroys the second leg irrecoverably. Verified against a synthetic templated deck (9 distinct slides, `candidates=0 -> STATIC`, differences plainly visible on the sheet). **Docs reconciled:** R10 corrected (was "two ingests, both video" - now 5 ingests, 4 video + 1 blog, with the blog flow exercised and paper/code still unrun), new R11 for the composited-recording risk, OQ3's stale "170 lines" refreshed to ~250 across 4 sources with the likely fault line named, §5.1 and the decisions table updated. | agent |
